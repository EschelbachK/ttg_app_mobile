...
final exerciseCatalogProvider = FutureProvider.autoDispose<List<ExerciseCatalogItem>>((ref) async {
  final api = ref.read(catalogApiProvider);
  final data = await api.fetchExercises();
...
class ExerciseCatalogNotifier extends StateNotifier<ExerciseCatalogState> {
  ...
  Future<void> fetchExercises({bool refresh = false}) async {
    ...
    try {
      ...
      if (state.bodyRegion != "ALL") filter['bodyRegion'] = state.bodyRegion;
      if (state.exerciseType != "ALL") filter['exerciseType'] = state.exerciseType;
