...
class _ExerciseSelectionCardState extends ConsumerState<ExerciseSelectionCard> {
  ...
  IconData getCategoryIcon(String category) => PhosphorIcons.barbell(PhosphorIconsStyle.fill);

  static const displayCategories = [
    "Alle Bereiche",
    "Brust",
    "Rücken",
    "Beine",
  ...
  Widget _label(String text) => Text(text, style: const TextStyle(color: Colors.white38, fontSize: 12));

  Widget _picker(String? value, Future<void> Function() onTap) => GestureDetector(
    onTap: onTap,
    child: Text(value ?? "Hier tippen", style: const TextStyle(color: Colors.white, fontSize: 18)),
  );

  Widget _divider() => const Divider(color: Colors.white12, height: 30);

  Widget _field(String title, String value, bool keyboard, bool decimal, VoidCallback onTap) {
    if (keyboard) {
          ...
          TextField(
            ...
            onChanged: (val) {
              ...
              if (decimal) {
                ...
                if (parsed != null) setState(() => weight = parsed);
              } else {
                final parsed = int.tryParse(val);
                if (parsed != null) setState(() => reps = parsed);
              }
            },
  ...
  Future<void> _pickExercise() async {
    ...
    final bodyRegion = mapCategoryToBodyRegion(category!);
    final result = await showModalBottomSheet<String>(
      context: context,
      backgroundColor: Colors.transparent,
      isScrollControlled: true,
      builder: (_) => FractionallySizedBox(
        heightFactor: 0.9,
        child: ExerciseSelectSheet(category: bodyRegion),
      ),
    );
    if (result != null) setState(() => exercise = result);
  }
  ...
  void _add() {
    ...
    final e = Exercise(
      ...
      name: exercise!,
      bodyRegion: category!,
      sets: List.generate(safeSets, (_) => ExerciseSet(weight: weight, reps: reps)),
    );
    ref.read(dashboardProvider.notifier).addExercise(widget.folderId, widget.planId, e);
